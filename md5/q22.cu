...
for(int k = 0; k < 2; k++){
	...
	record_t *h_z[2][S_CHUNK_CNT];
	int zlen[2][S_CHUNK_CNT];

	gettimeofday(&t1, NULL);
	for(int i = 0; i < S_CHUNK_CNT; i++){
		CUDA_SAFE_CALL(cudaMallocHost((void**)&h_z[0][i], sizeof(record_t) * S_CHUNK_LEN));
		zlen[0][i] = read_s(h_z[0][i], S_CHUNK_LEN, 0);
		CUDA_SAFE_CALL(cudaMallocHost((void**)&h_z[1][i], sizeof(record_t) * S_CHUNK_LEN));
		zlen[1][i] = read_s(h_z[1][i], S_CHUNK_LEN, 0);
	}
	gettimeofday(&t2, NULL);
...
for(int k = 0; k < 2; k++) {
	for(int i = 0; i < S_CHUNK_CNT; i++) {
		...
		if(slen[k][i] > 0) {
			...
			if(hash_join(NULL, NULL, &ht_r[k], h_s[k][i], slen[k][i], stream, S_CHUNK_CNT+i)) {
			...
			for(int j = 0; j < S_CHUNK_CNT; j++) {
				if(zlen[k][j] > 0) {
					if(hash_join(NULL, NULL, &ht_r[k], h_z[k][j], zlen[k][j], stream, S_CHUNK_CNT+i)) {
