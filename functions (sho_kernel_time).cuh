 ...
 //    }
   ...
   //  		}
			...
			vectorAdd<<<Kernel.GridDim, Kernel.BlockDim, 
				0, Streams[StreamID]>>>(D_data, D_result, Kernel.dataSize);
...
extern void initKernel(Kernel_info *Kernel, int kernelMode, int Thread, int DataSize){
  ...
  Kernel[5].dataSize = Kernel[5].BlockDim.x * Kernel[5].GridDim.x;
  if(MaxDataSize <= Kernel[5].dataSize){
      MaxDataSize = Kernel[5].dataSize;
  }
  ...
  Kernel[6].dataSize = Kernel[6].BlockDim.x * Kernel[6].GridDim.x;
  if(MaxDataSize <= Kernel[6].dataSize){
      MaxDataSize = Kernel[6].dataSize;
  }
  ...
  Kernel[7].dataSize = Kernel[7].BlockDim.x * Kernel[7].GridDim.x;
  if(MaxDataSize <= Kernel[7].dataSize){
      MaxDataSize = Kernel[7].dataSize;
  }
  ...
  Kernel[8].dataSize = Kernel[8].BlockDim.x * Kernel[8].GridDim.x;
  if(MaxDataSize <= Kernel[8].dataSize){
      MaxDataSize = Kernel[8].dataSize;
  }
  ...
  Kernel[9].dataSize = Kernel[9].BlockDim.x * Kernel[9].GridDim.x;
  if(MaxDataSize <= Kernel[9].dataSize){
      MaxDataSize = Kernel[9].dataSize;
  }
  ...
  Kernel[10].dataSize = Kernel[10].BlockDim.x * Kernel[10].GridDim.x;
  if(MaxDataSize <= Kernel[10].dataSize){
      MaxDataSize = Kernel[10].dataSize;
  }
	...
	Kernel[11].sharedMem = 0;
  Kernel[11].dataSize = Kernel[11].BlockDim.x * Kernel[11].GridDim.x;
  if(MaxDataSize <= Kernel[11].dataSize){
      MaxDataSize = Kernel[11].dataSize;
  }
	...
	Kernel[12].sharedMem = 0;
  Kernel[12].dataSize = Kernel[12].BlockDim.x * Kernel[12].GridDim.x;
  if(MaxDataSize <= Kernel[12].dataSize){
      MaxDataSize = Kernel[12].dataSize;
  }
...
extern float concurrentRun(int TotalKernelNum, int *KernelNum, Kernel_info *Kernel, 
     ...
     *                    cudaStream_t* Streams, int StreamID);
     */
		lauchKernel(KernelNum[i], Kernel[KernelNum[i]], D_data, D_result, Streams, i);
	}
