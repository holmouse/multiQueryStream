...
int read_r(record_t **r_tmp, int *rlen)
{
	...
	// }
	// record_t *r_tmp;
	res = cudaMallocHost((void**)&(*r_tmp), size_r);
	if(res != cudaSuccess) {
		fprintf(stderr, "malloc failed for R\n");
		return -1;
	}
...
int main()
{
	...
	if(build_hash_table(&ht_r, h_r, rlen, stream, 0)) {
	...
	res = cudaMallocHost((void**)&h_s, sizeof(record_t) * S_CHUNK_LEN);
	if(res != cudaSuccess) {
		fprintf(stderr, "malloc failed for h_s\n");
		return -1;
	}
	...
	for(int i = 0; i < S_CHUNK_CNT; i++) {
		...
		if(slen > 0) {
			...
			if(hash_join(NULL, NULL, &ht_r, *h_s, slen, stream, i)) {
