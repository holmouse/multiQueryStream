...
int main()
{
	...
	for(int i = 0; i < S_CHUNK_CNT; i++) {
		h_z[i] = (record_t *)malloc(sizeof(record_t) * S_CHUNK_LEN);
		if(!h_z[i]) {
			fprintf(stderr, "malloc failed for z\n");
			free_hash_table(&ht_r);
			return -1;
		}
	}
	...
	gettimeofday(&t2, NULL);
	printf(	"Time on reading Z: %lf ms ( %lf ms per read )\n", TIME_DIFF(t1, t2), TIME_DIFF(t1, t2)/ S_CHUNK_CNT);
	...
	for(int i = 0; i < S_CHUNK_CNT; i++) {
		...
		if(slen > 0) {
			...
			for(int j = 0; j < S_CHUNK_CNT; j++) {
				if(zlen[j] > 0) {
					if(hash_join(NULL, NULL, &ht_r, h_z[j], zlen[j])) {
						fprintf(stderr, "hash join failed for the %dth chunk of S\n", j);
						break;
					}
				}
