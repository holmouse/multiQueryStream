...
extern float concurrentRun(int TotalKernelNum, int *KernelNum, bool *KernelSync, Kernel_info *Kernel, 
	float *D_data, float *D_result, cudaStream_t *Streams){
...
extern int benchmarkRun(int Num, int Thread, int Datasize){
  ...
  kernelNum = (int *)malloc(sizeof(int) * Num);
  bool *kernelSync;
  kernelSync = (bool *)malloc(sizeof(bool) * Num);
...
printf("time:%f ms\n", time_spent*1000);
  ...
  while(count < Num){
    ...
    MaxWarp -= ceil(kernel[kernelNum[count]].dataSize / 32);
    MaxReg -= ceil(ceil(kernel[kernelNum[count]].dataSize / 32)*32/8);
    MaxSM -= kernel[kernelNum[count]].sharedMem * kernel[kernelNum[count]].GridDim.x;
    if( MaxWarp <= 0 || MaxReg <= 0 || MaxSM <= 0 ){
      kernelSync[count] = true;
      MaxWarp = 64 * 14;
      MaxReg = 65536 / 8 * 14;
      MaxSM = 49152 * 14;
    }
  ...
  printf("%f\t", concurrentRun(Num, kernelNum, kernelSync, kernel, d_data, d_result, streams));
...
if (error != cudaSuccess)
  ...
  printf("%f\n", concurrentRun(Num, kernelNum, kernelSync, kernel, d_data, d_result, streams));
